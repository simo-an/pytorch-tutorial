...
import transforms
from structure import FasterRCNN
...
def create_model(num_classes):
    ...
    vgg_feature = make_model(model_name="vgg16", weights_path="./backbone/vgg16_model.pth").features
    # 删除features中最后一个Maxpool层
...
def main():
    ...
    # check voc root
    if os.path.exists(os.path.join(DEF_root, "NEU-DET")) is False:
        raise FileNotFoundError(
            "NEU-DET dose not in path:'{}'.".format(DEF_root))

    # load train data set
    # NEU-DET -> Config -> train.txt
    train_dataset = DefectDataset(
        DEF_root, data_transform["train"], "train.txt")
    train_sampler = None
    ...
    # NEU-DET -> Config -> val.txt
    val_dataset = DefectDataset(DEF_root, data_transform["val"], "val.txt")
    ...
    optimizer = torch.optim.SGD(params, lr=0.005,
                                momentum=0.9, weight_decay=0.0005)
    # learning rate scheduler 每隔3步变为原来的0.33
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        # save weights
        # 仅保存最后2个epoch的权重
        if epoch in range(num_epochs+init_epochs)[-2:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(
                save_files, "./save_weights/vgg16-model-{}.pth".format(epoch))
